// FILE: src/app/api/forecast/train/route.ts
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!service) {
            return NextResponse.json({ error: "Missing service in body" }, { status: 400 });
        }
        const encoded = encodeURIComponent(service);
        const res = await fetch(`${BACKEND_BASE}/train/${encoded}`, {
            method: "POST",
        ...
        return NextResponse.json({ error: "Failed to trigger training", detail: err?.message ?? String(err) }, { status: 500 });
    }
}
